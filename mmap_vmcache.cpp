...
struct PageState {
   ...
   static const u64 MaxShared = 254;
...
BufferManager::BufferManager() : virtSize(envOr("VIRTGB", 16)*gb), virtCount(virtSize / pageSize) {
   ...
   cerr << "mmap_vmcache " << "blk:" << path << " virtgb:" << virtSize/gb << endl;
}
...
int main(int argc, char** argv) {
   ...
   auto statFn = [&]() {
      ...
      for (uint64_t i=0; i<runForSec; i++) {
         ...
         cout << cnt++ << "," << prog << "," << 0 << "," << 0 << "," << systemName << "," << nthreads << "," << n << "," << (isRndread?"rndread":"tpcc") << "," << 0 << endl;
      }
      keepRunning = false;
   };
